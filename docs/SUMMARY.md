# Table of contents

* [Introduction](README.md)
* [Authentication with Azure AD](authentication.md)
* [Conceptual bits that are useful for debugging Archivematica](conceptual-bits.md)
* [user\_management](user\_management.md)
* [developers](developers/README.md)
  * [Why do we fork Archivematica? How do our forks work?](developers/archivematica-forks.md)
  * [Bootstrapping a clean Archivematica stack](developers/bootstrapping.md)
  * [How we handle external identifiers in Archivematica](developers/external-identifiers.md)
  * [Troubleshooting](developers/troubleshooting.md)
* [howto](howto/README.md)
  * [How to add or remove users](howto/add-or-remove-users.md)
  * [How to create a transfer package](howto/create-a-transfer-package.md)
  * [SSH into the Archivematica container hosts](howto/ssh-into-container-hosts.md)

## Storing born-digital files

* [Creating a transfer package](storing-born-digital-files/creating-a-transfer-package.md)
* [Upload a transfer package to S3](storing-born-digital-files/upload-a-transfer-package-to-s3.md)
* [Check a package was stored successfully](storing-born-digital-files/check-a-transfer-package-is-stored.md)
* [Downloading a package from the storage service](storing-born-digital-files/downloading-a-package-from-the-storage-service.md)

## Service architecture

* [What are the Archivematica apps?](service-architecture/archivematica-apps.md)
* [What are our extra services?](service-architecture/what-are-our-extra-services.md)
* [Unusual aspects of our deployment](service-architecture/unusual-aspects-of-our-deployment.md)
* [How it fits into the wider platform](service-architecture/how-it-fits-into-the-wider-platform.md)
