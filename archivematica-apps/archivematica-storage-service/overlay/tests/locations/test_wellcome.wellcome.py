...
# Stored in the METS file of the small_compressed_bag.tar.gz file.
EXTERNAL_IDENTIFIER = "ARTCOO/B/17"


def create_storage_service_response(status):
    resp = {
        "id": str(uuid.uuid4()),
        "callback": {"status": {"id": status}},
        "status": {"id": status},
        "events": [
            {"type": "IngestEvent", "description": "Something happened"},
            {"type": "IngestEvent", "description": "Something else happened"},
        ],
        "bag": {"info": {"externalIdentifier": str(uuid.uuid4())}},
    }
...
class WellcomeTestBase(TestCase):
    fixture_files = ["base.json", "wellcome.json"]
    fixtures = [os.path.join(FIXTURES_DIR, f) for f in fixture_files]
...
class TestWellcomeMoveFromStorageService(WellcomeTestBase):
    def setUp(self):
        super().setUp()
        shutil.copy(
            os.path.join(FIXTURES_DIR, "small_compressed_bag.tar.gz"), self.tmp_dir
        )
    ...
    def test_uploads_bag_to_s3_bucket(self, mock_wellcome_client_class, mock_sleep):
        ...
        self.wellcome_object.move_from_storage_service(
            os.path.join(self.tmp_dir, "small_compressed_bag.tar.gz"),
            "/born-digital/bag.tar.gz",
            package=package,
        )

        assert self._s3.get_object(
            Bucket="ingest-bucket", Key="born-digital/bag.tar.gz"
        )
    ...
    def test_calls_wellcome_ss_client(self, mock_wellcome_client_class, mock_sleep):
        ...
        mock_wellcome_client_class.return_value.create_s3_ingest.assert_called_with(
            space="born-digital",
            s3_key="born-digital/bag.tar.gz",
    ...
    def test_tries_fetching_ingest_if_no_callback(
        ...
        package.current_path = (
            "locations/fixtures/bag-6465da4a-ea88-4300-ac56-9641125f1276.tar.gz"
        )
        ...
        assert package.current_path == "bag-6465da4a-ea88-4300-ac56-9641125f1276.tar.gz"
        assert (
    ...
    def test_tries_fetching_failed_ingest_if_no_callback(
        ...
        path = "locations/fixtures/bag-6465da4a-ea88-4300-ac56-9641125f1276.tar.gz"
        ...
        with pytest.raises(models.StorageException, match="Failed to store package"):
            self.wellcome_object.move_from_storage_service(
                src_path=os.path.join(self.tmp_dir, "small_compressed_bag.tar.gz"),
                dest_path="/born-digital/bag.tar.gz",
                package=package,
            )
...
def test_extract_dc_identifiers(mets_xml, expected_identifiers, tmp_path):
    p = tmp_path / "mets.xml"
    p.write_bytes(mets_xml.strip())
    assert list(extract_dc_identifiers(str(p))) == expected_identifiers
...
@pytest.mark.parametrize(
    ...
    [
        ...
        <?xml version='1.0' encoding='UTF-8'?>
        <mets:mets xmlns:mets="http://www.loc.gov/METS/">
          <mets:altRecordID TYPE="Accession ID">LEMON/1234</mets:altRecordID>
        </mets:mets>
        """,
            ["LEMON/1234"],
        ),
        # It ignores a mets:altRecordIDType that isn't "Accession ID"
        (
            b"""
        <?xml version='1.0' encoding='UTF-8'?>
        <mets:mets xmlns:mets="http://www.loc.gov/METS/">
          <mets:altRecordID TYPE="Numeric ID">12345</mets:altRecordID>
          <mets:altRecordID TYPE="Accession ID">LEMON/1234</mets:altRecordID>
        </mets:mets>
        """,
            ["LEMON/1234"],
        ...
        <mets:mets xmlns:mets="http://www.loc.gov/METS/">
          <mets:altRecordID TYPE="Accession ID">LEMON/1234</mets:altRecordID>
          <mets:altRecordID TYPE="Accession ID">LEMON/1234/5</mets:altRecordID>
        </mets:mets>
        """,
...
def test_extract_accession_identifiers(mets_xml, expected_identifiers, tmp_path):
    p = tmp_path / "mets.xml"
    p.write_bytes(mets_xml.strip())
    assert list(extract_accession_identifiers(str(p))) == expected_identifiers
