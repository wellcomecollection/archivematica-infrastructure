...
@mock.patch(
    ...
    return_value=mock.Mock(
        **{
            "meta.client.head_bucket.return_value": {
...
@mock.patch(
    ...
    return_value=mock.Mock(
        **{
            "meta.client.head_bucket.side_effect": botocore.exceptions.ClientError(
...
@mock.patch(
    "boto3.resource",
    return_value=mock.Mock(
        **{"meta.client.head_bucket.side_effect": botocore.exceptions.BotoCoreError}
    ),
)
...
def test_ensure_bucket_exists_works_with_custom_endpoint_url(resource, s3_space):
    s3_space.aws_access_key_id = "minio"
    s3_space.aws_secret_access_key = "minio123"
    s3_space.s3_region = "planet-earth"
    s3_space.s3_endpoint_url = "https://localhost:9000"
    s3_space.save()
...
def test_ensure_bucket_exists_works_with_any_region(resource, s3_space):
    s3_space.s3_region = "ca-central-1"
    s3_space.save()
    ...
    resource.return_value.create_bucket.assert_called_once_with(
        ...
        CreateBucketConfiguration={"LocationConstraint": s3_space.s3_region},
    )
