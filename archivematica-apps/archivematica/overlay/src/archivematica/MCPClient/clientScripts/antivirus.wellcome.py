#!/usr/bin/env python
"""
Wellcome note: we've had a number of issues with the virus scanning step
where this script throws a MySQL error:

    OperationalError: (1040, 'Too many connections')

We've made a couple of changes to this script to hopefully reduce the
database load and make this step more reliable.

In particular, the original script favours getting cached results from
the database over repeating work.  This makes sense in an environment
where you might be processing the same files multiple times.

At Wellcome, our transfer packages only run through Archivematica once --
it's extremely unlikely there would be any cached results to fetch, so
we skip trying to find any.  Our hope is that this reduces database load.
...
def file_already_scanned(file_uuid):
    # In the Wellcome version of Archivematica, it would be very unusual
...
def get_size(file_uuid, path):
    ...
    # We skip the database lookup for Wellcome.
    # Our fallback.
    try:
